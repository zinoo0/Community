      ...
      <v-row>
        <v-col cols="12" md="2">
          <v-list-item @click="realyagu()">
            <img src="https://ifh.cc/g/wn4U0.jpg" width="160" height="200" />
          </v-list-item>
          <br />
          <v-list-item @click="realyagu()">
            <img src="https://ifh.cc/g/FL171.png" width="160" height="200" />
          </v-list-item>
          <br />
        ...
        <v-card>
          ...
          <v-card-text text color="deep-purple accent-4">
            ...
            <v-list-item v-for="(comment, index) in comments" :key="index" :max-width="900">
              <v-card width="900">
                <v-list-item>
                  최진호 {{ $moment(comment.reportingDate).format('YY-MM-DD HH:mm') }}
                  <v-spacer></v-spacer>
                  <v-menu right>
...
export default {
  data: () => ({
    ...
    dataURL: "http://localhost:1225/community",
    no: 1,
    pageSize: 10,
    count: 1,
    commentcount: 1,
  ...
  methods: {
    ...
    add() {
      ...
      this.$axios
        .post("http://localhost:1225/community/boards", this.board)
        .then(result => {
    ...
    comment2() {
      if (this.comment != "") {
        this.$axios
          .post("http://localhost:1225/community/boards/comments", {
    ...
    select(row) {
      ...
      this.$axios
        .get(
          `http://localhost:1225/community/boards/${
      ...
      this.$axios
        .get(
          `http://localhost:1225/community/boards/news/${this.selectedboard.line}`
        )
        .then(result => {
      ...
      this.$axios
        .put("http://localhost:1225/community/boards", this.selectedboard)
        .then(() => {});
    ...
    del() {
      this.$axios
        .delete(
          `http://localhost:1225/community/boards/${this.selectedboard.no}`
        )
        .then(() => {
    ...
    del0(id, index) {
      ...
      this.$axios
        .delete(`http://localhost:1225/community/boards/comments/${id}`)
        .then(() => {
    ...
    save() {
      this.$axios
        .put("http://localhost:1225/community/boards", this.edittingboard)
        .then(() => {
          var index = this.boards.indexOf(this.selectedboard);
          this.$set(
            this.boards,
    ...
    save0() {
      this.$axios
        .put(
          "http://localhost:1225/community/boards/comments",
  ...
  created() {
    this.boards = [];
    this.$axios
      .get("http://localhost:1225/community/boards", {})
      .then(result => {
        this.boards = result.data.reverse();
      });
    this.$axios
      .get("http://localhost:1225/community/ranking", {})
      .then(result => {
